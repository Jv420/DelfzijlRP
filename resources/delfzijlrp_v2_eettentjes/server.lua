...
local function findStand(standId)
    for _, stand in ipairs(Config.Stands) do
        if stand.id == standId then return stand end
    end
    return nil
end

local function findItem(stand, itemName)
...
lib.callback.register('delfzijlrp_v2_eettentjes:server:buy', function(source, standId, itemName, amount, tip)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return false, Config.Text.invalid end

    amount = tonumber(amount) or 1
    tip = tonumber(tip) or 0
    if amount < 1 or amount > Config.Extras.maxAmount then return false, Config.Text.invalid end
    if tip < 0 or tip > 500 then return false, Config.Text.invalid end

    local stand = findStand(standId)
    if not stand then return false, Config.Text.invalid end
    local item = findItem(stand, itemName)
